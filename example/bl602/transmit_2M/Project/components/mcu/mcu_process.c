...
static bool  cmdProcessState = false;
...
static void McuProcessTask(void *arg)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            // printf("0x%02x ", uart_cmd_buffer[cmdlen-1]);
            // if(uart_cmd_buffer[cmdlen-1] == 0xa5 && uart_cmd_buffer[cmdlen - 2] == 0x5a)
            // {
            //     cmdProcessState = true;
            // }

            // if((cmdlen >= 7) && cmdProcessState && (UartCheckValue((uint8_t *)uart_cmd_buffer, cmdlen -1) == uart_cmd_buffer[cmdlen - 1]))
            // {
