...
static char Messagebuffer[UUID4_STR_BUFFER_SIZE];
// —————————————————————————————————————————————————————————————————————————————
...
void UUID4_PREFIX(seed)(uint64_t* state)
{
  ...
  uint32_t time = ailinkGetSystemTick();

  *state = state0++ + time;

  uint32_t pid = (uint32_t)AilinkGetRand();
  uint64_t tid = (uint32_t)AilinkGetRand();
  // pthread_threadid_np(NULL, &tid);
  *state = *state * 6364136223846793005u + ((uint64_t)(UUID4_PREFIX(mix)(UUID4_PREFIX(hash)(pid), UUID4_PREFIX(hash)((uint32_t)tid))) << 32);
  *state = *state * 6364136223846793005u + (uintptr_t)AilinkGetRand;
  *state = *state * 6364136223846793005u + (uintptr_t)UUID4_PREFIX(gen);
}
...
char *GetMessageId(void)
{
    ...
    if (!uuid4_to_s(uuid, Messagebuffer, sizeof(Messagebuffer)))
    {
      return NULL;
    }
